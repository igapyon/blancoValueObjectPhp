...
/**
 * バッチ処理クラス [BlancoValueObjectPhpBatchProcess]。
 *
 * <P>バッチ処理の呼び出し例。</P>
 * <code>
 * java -classpath (クラスパス) blanco.valueobjectphp.task.BlancoValueObjectPhpBatchProcess -help
 * </code>
 */
public class BlancoValueObjectPhpBatchProcess {
    /**
     * 正常終了。
     */
    ...
    public static final void main(final String[] args) {
        final BlancoValueObjectPhpBatchProcess batchProcess = new BlancoValueObjectPhpBatchProcess();

        // バッチ処理の引数。
        final BlancoValueObjectPhpProcessInput input = new BlancoValueObjectPhpProcessInput();
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (arg.startsWith("-verbose=")) {
                ...
                System.out.println("BlancoValueObjectPhpBatchProcess: 入力パラメータ[" + arg + "]は無視されました。");
                isNeedUsage = true;
            }
        }
        ...
        if( isFieldMetadirProcessed == false) {
            System.out.println("BlancoValueObjectPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
     ...
     */
    public int process(final BlancoValueObjectPhpProcessInput input) throws IOException, IllegalArgumentException {
        // 入力パラメータをチェックします。
        validateInput(input);

        // この箇所でコンパイルエラーが発生する場合、BlancoValueObjectPhpProcessインタフェースを実装して blanco.valueobjectphp.taskパッケージに BlancoValueObjectPhpProcessImplクラスを作成することにより解決できる場合があります。
        final BlancoValueObjectPhpProcess process = new BlancoValueObjectPhpProcessImpl();
     ...
     * @throws IllegalArgumentException 入力値に不正が見つかった場合。
     */
    public final int execute(final BlancoValueObjectPhpProcessInput input) throws IllegalArgumentException {
        try {
            // バッチ処理の本体を実行します。
            ...
            System.out.println("BlancoValueObjectPhpBatchProcess: 入力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_ILLEGAL_ARGUMENT_EXCEPTION;
        } catch (IOException ex) {
            System.out.println("BlancoValueObjectPhpBatchProcess: 入出力例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            // 入力異常終了。
            return END_IO_EXCEPTION;
        } catch (RuntimeException ex) {
            System.out.println("BlancoValueObjectPhpBatchProcess: ランタイム例外が発生しました。バッチ処理を中断します。:" + ex.toString());
            ex.printStackTrace();
            // 異常終了。
            return END_ERROR;
        } catch (Error er) {
            System.out.println("BlancoValueObjectPhpBatchProcess: ランタイムエラーが発生しました。バッチ処理を中断します。:" + er.toString());
            er.printStackTrace();
            // 異常終了。
            return END_ERROR;
        }
    ...
    public static final void usage() {
        System.out.println("BlancoValueObjectPhpBatchProcess: Usage:");
        System.out.println("  java blanco.valueobjectphp.task.BlancoValueObjectPhpBatchProcess -verbose=値1 -metadir=値2 -targetdir=値3 -tmpdir=値4 -nameAdjust=値5 -encoding=値6");
     ...
     * @throws IllegalArgumentException 入力値に不正が見つかった場合。
     */
    public void validateInput(final BlancoValueObjectPhpProcessInput input) throws IllegalArgumentException {
        if (input == null) {
        ...
        if (input.getMetadir() == null) {
            throw new IllegalArgumentException("BlancoValueObjectPhpBatchProcess: 処理開始失敗。入力パラメータ[input]の必須フィールド値[metadir]に値が設定されていません。");
        }
    }
}
