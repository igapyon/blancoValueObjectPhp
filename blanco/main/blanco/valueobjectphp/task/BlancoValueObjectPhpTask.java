...
/**
 * Apache Antタスク [BlancoValueObjectPhp]のクラス。
 *
 * バリューオブジェクト定義書からバリューオブジェクトクラスを自動生成するためのAntTaskです。<br>
 * このクラスでは、Apache Antタスクで一般的に必要なチェックなどのコーディングを肩代わりします。
 * 実際の処理は パッケージ[blanco.valueobjectphp.task]にBlancoValueObjectPhpBatchProcessクラスを作成して記述してください。<br>
 * <br>
 * Antタスクへの組み込み例<br>
 * <pre>
...
public class BlancoValueObjectPhpTask extends Task {
    /**
     * バリューオブジェクト定義書からバリューオブジェクトクラスを自動生成するためのAntTaskです。
     */
